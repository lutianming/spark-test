 ...
 */
@DeveloperApi
abstract class MyGeneralizedLinearModel(val weights: Vector, val intercept: Double)
 ...
 */
@DeveloperApi
abstract class MyGeneralizedLinearAlgorithm[M <: MyGeneralizedLinearModel]
  extends Logging with Serializable {

  protected val validators: Seq[RDD[LabeledPoint] => Boolean] = List()

  /** The optimizer to solve the problem. */
  def optimizer: MyOptimizer
   ...
   */
  def run(input: RDD[LabeledPoint]): (M, Array[Double], Array[Long])= {
   ...
   */
  def run(input: RDD[LabeledPoint], initialWeights: Vector): (M, Array[Double], Array[Long]) = {
    ...
    val (weightsWithIntercept, loss, iters) = optimizer.optimize(data, initialWeightsWithIntercept)
    ...
    (createModel(weights, intercept), loss, iters)
  }
}
