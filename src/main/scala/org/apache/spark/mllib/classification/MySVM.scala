...
class SVMModelMy (
  ...
  extends MyGeneralizedLinearModel(weights, intercept) with ClassificationModel with Serializable {
 ...
 * regularization is used, which can be changed via [[MySVMWithSGD.optimizer]].
...
class MySVMWithSGD private (
  ...
  override val optimizer = new MyGradientDescent(gradient, updater)
